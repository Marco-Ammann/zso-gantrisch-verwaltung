...
export class AuthService {
  ...
  constructor() {
    // Beim Start den Authentifizierungsstatus überwachen
  ...
  async updateUserRole(uid: string, newRole: User['role']): Promise<void> {
    try {
      const users = await this.firebaseService.query<User & { id: string }>('users', 'uid', '==', uid);
      ...
      if (users && users.length > 0) {
        ...
        if (currentUser && currentUser.uid === uid) {
          this._currentUser.update(user => user ? { ...user, role: newRole } : null);
        }
      }
    } catch (error) {
